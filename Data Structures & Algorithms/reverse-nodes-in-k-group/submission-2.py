...
class Solution:
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        if group == k:
            ...
            prev = self.reverseKGroup(curr, k) # This should be k+1 node if the next recursion is not equal to k
            while group > 0:
                # Store the next node
                temp = head.next
                ...
                head.next = prev
                # Swapping 
                prev = head # Storing the next node for the next round to connect
                head = temp
                group -= 1
            head = prev
        return head
