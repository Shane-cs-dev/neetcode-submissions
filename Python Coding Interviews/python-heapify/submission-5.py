...
def heapify_strings(strings: List[str]) -> List[str]:
    ...
    return [heapq.heappop(strings) for i in range(len(strings))]



def heapify_integers(integers: List[int]) -> List[int]:
    heapq.heapify(integers)
    return [heapq.heappop(integers) for i in range(len(integers))]


def heap_sort(nums: List[int]) -> List[int]:
    heapq.heapify(nums)
    return [heapq.heapify(nums) for i in range(len(nums))]
